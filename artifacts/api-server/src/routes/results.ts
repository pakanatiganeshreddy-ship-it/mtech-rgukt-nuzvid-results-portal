...
const GRADE_POINTS: Record<string, number> = {
  ...
  R: 0,      // Reappear — treated as fail (0 points) until cleared
  FAIL: 0,
  "FAIL(R)": 0,
  F: 0,
};
 ...
 *   48 NM2504TE06 Semester-1 CE 21TE1112 Transportation Engineering Road Safety 3 R April,2026 2025
 */
function parseRGUKTLine(line: string): ExtractedRecord | null {
  ...
  const endPattern = new RegExp(
    `(\\d+(?:\\.\\d+)?)\\s+(EX|Ex|A|B|C|D|E|R|P|Fail(?:\\s*\\(?R\\)?)?)\\s+(?:${MONTHS}),?\\s*\\d{4}\\s+(\\d{4})\\s*$`,
    "i"
  );
